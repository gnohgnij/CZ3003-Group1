...
public class CreateAssignmentController : MonoBehaviour
{
    ...
    {
        StartCoroutine(Submit_Assignment(DDInputField.text, MMInputField.text, YYYYInputField.text, hhInputField.text, mmInputField.text, ssInputField.text));
    }

    private IEnumerator Submit_Assignment(string dd, string MM, string yyyy, string HH, string mm, string ss)
    {
        WarningText.gameObject.SetActive(false);
        if (StateManager.assignmentQuestionSize == 0)
        {
